...
const regex = /const \{ data, error \} = await supabase\.from\('volunteer_sessions'\)\.select\('\*'\);\s+if \(error\) \{\s+console\.error\("Error fetching volunteers: ", error\);\s+\} else \{\s+setVolunteers\(data\);\s+\}/;
